    ...
    if (req.method === "DELETE") {
      ...
      if (results) {
        const insert = database.prepare(
          "UPDATE notes set notes = ?, move = ?, repertoire = ? WHERE fen = ?",
        );
        // Execute the prepared statement with bound values.

        console.log("Body we are about to insert", body);
        insert.run(body.notes, body.move, body.repertoire, body.fen);
        // Create a prepared statement to read data from the database.
      ...
      const insert = database.prepare(
        "INSERT INTO notes (fen, notes, move, repertoire, original_location) VALUES (?, ?, ?, ?, ?)",
      );
      // Execute the prepared statement with bound values.
