...
export default function getTVPlan(inputPGN: ParsedPGN): TVPlan {
    ...
    let blackClockElapsed = timeControlIncrement, whiteClockElapsed = timeControlIncrement;
    ...
    const newMoves: TimedMove[] = inputPGN.moves.map((move: pgnParser.Move, index: number) => {
        ...
        const totalTimeWithIncrements = (timeControlBase) + Math.round(index/2) * timeControlIncrement; 

        const totalCurrent = hour * 60 * 60 + minute * 60 + second;

        const elapsedSeconds = 
            totalTimeWithIncrements - totalCurrent;
        ...
        return {
            ...
            move: move.move,
            player: move.move_number && move.move_number % 2 === 0 ? 'black' : 'white',
            whiteClockSeconds: whiteClockElapsed,
            blackClockSeconds: blackClockElapsed
        }
    });
