...
export default function getGameFromPGN(
  inputPGN: ParsedPGN,
  origin?: string,
): Game | null {
  ...
  if (headers) {
    ...
    var someDate = new Date(
      headersObject["UTCDate"] + " " + headersObject["UTCTime"] + "Z",
    );
    let end_time = someDate.getTime() / 1000;

    if (isNaN(end_time)) {
      // probably Safari
      someDate = new Date(
        headersObject["UTCDate"].replaceAll(".", "-") +
          " " +
          headersObject["UTCTime"] +
          "Z",
      );
      end_time = someDate.getTime() / 1000;
    }
