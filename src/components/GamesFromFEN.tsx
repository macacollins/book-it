...
const GamesFromFEN: React.FC<GamesFromFENProps> = ({ initialFEN }) => {
  ...
  const [masterData, setMasterData] = useState<OpeningExplorerMasters | null>(
    null,
  );
  ...
  const loadLichessData = async () => {
    ...
    try {
      ...
      setError(
        `Failed to load data from Lichess: ${err instanceof Error ? err.message : "Unknown error"}`,
      );
    } finally {
      setLoading(false);
    }
  ...
  const moveTemplate = (move: any) => {
    return (
      <div style={{ padding: "0.5rem" }}>
        ...
        <div
          style={{ fontSize: "0.9rem", color: "var(--text-color-secondary)" }}
        >
  ...
  return (
    <div className="games-from-fen" style={{ padding: "1rem" }}>
      ...
      <Panel header="Position Setup" className="mb-4">
        <FENChooser initialFEN={currentFEN} onFENChange={handleFENChange} />
      ...
      {masterData && (
        <div>
          ...
          {masterData.moves && masterData.moves.length > 0 && (
            <Panel
              header={`Popular Moves (${masterData.moves.length})`}
              className="mb-4"
            >
              <div
                style={{
          ...
          {masterData.topGames && masterData.topGames.length > 0 && (
            <Panel
              header={`Master Games (${masterData.topGames.length})`}
              className="mb-4"
            >
              <DataTable
