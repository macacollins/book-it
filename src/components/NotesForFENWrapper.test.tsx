...
jest.mock('./FENChooser', () => {
  return function MockFENChooser({ onFENChange }: { onFENChange: (fen: string) => void }) {
    return (
...
describe('NotesForFENWrapper', () => {
  ...
  it('includes usage instructions for all tabs', () => {
    ...
    expect(screen.getByText(/Notes tab.*Enter notes in the text area/)).toBeInTheDocument();
    expect(screen.getByText(/Masters tab.*View statistics from the Lichess masters database/)).toBeInTheDocument();
    expect(screen.getByText(/Lichess Stats tab.*View statistics from Lichess games/)).toBeInTheDocument();
  });
});