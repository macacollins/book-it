function saveCache(fen: string, lichessResults: any) {
  localStorage.setItem(fen, JSON.stringify(lichessResults));
}

function retrieveCache(fen: string) {
...
async function getLichessAnalysis(fen: string) {
  let params =
    "variant=standard" +
    `&fen=${fen}` +
    "&speeds=rapid,blitz" +
