 ...
 */
export const createMoveTree = (name: string = "", headers: Record<string, string> = {}): MoveTree => ({
  name,
  headers,
  nodes: []
});
 ...
 */
export const findStartNode = (moveTree: MoveTree, fen: string): StartNode | null => {
 ...
 */
export const findMoveNode = (nodes: MoveNode[], targetFEN: string): MoveNode | null => {
 ...
 */
export const findNodeByFEN = (moveTree: MoveTree, fen: string): { type: 'start' | 'move', node: StartNode | MoveNode } | null => {
  // Check start nodes
...
export const moveTreeToJSON = (moveTree: MoveTree): MoveTreeJSON => {
  const convertMoveNode = (node: MoveNode): any => ({
    move: node.move,
    fen: node.fen,
    children: node.children.length > 0 ? node.children.map(convertMoveNode) : undefined
  });
 ...
 */
export const generateMoveTreeFromPGN = (pgn: string, startingFEN: string = 'rnbqkbnr/pppppppp/8/8/8/8/PPPPPPPP/RNBQKBNR w KQkq - 0 1', treeName: string = ''): MoveTree => {
  ...
  try {
    ...
    if (parsed && parsed.length > 0) {
      ...
      if (game.moves && game.moves.length > 0) {
        ...
        const processParsedMoves = (moves: any[], parentFEN: string, currentColor: 'w' | 'b', moveNumber: number, halfMoveCount: number): void => {
          ...
          for (const moveItem of moves) {
            ...
            if (typeof moveItem === 'string') {
              move = moveItem;
            } else if (moveItem && typeof moveItem === 'object' && moveItem.move) {
              move = moveItem.move;
            } else {
              continue;
            }
 ...
 */
export const editMoveTreeName = (moveTree: MoveTree, newName: string): MoveTree => {
  return {
    ...moveTree,
    name: newName
  };
