...
function calculateAnalysis(
  ...
  let realPlayerName =
    headers["Site"]?.indexOf("lichess") === -1 ? playerName : lichessPlayerName;
  ...
  let finalArrows: ArrowConfig[] = calculateGreenArrows(
    repertoireMoves,
    lastFEN,
    invert_board,
  );
  ...
  const chessGameDisplay = calculateFinalPosition(
    lastFEN,
    foundIntersection,
    game,
  );
