import {
  DrillCompletionData,
  DrillProgressHistory,
} from "../types/DrillProgress";
 ...
 */
export const saveDrillCompletion = (
  completionData: DrillCompletionData,
): void => {
 ...
 */
export const getLastDrillCompletion = (
  filename: string,
): DrillCompletionData | null => {
  ...
  for (let i = 0; i < localStorage.length; i++) {
    ...
    if (
      ...
      (key.startsWith(COMPLETION_KEY_PREFIX) ||
        key.startsWith(COMPLETION_HISTORY_KEY_PREFIX))
    ) {
