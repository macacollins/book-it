...
export function ChannelForm({ repertoire }: { repertoire: Repertoire }) {
  ...
  const previewFENSection = previewFENs.map((fen) => (
    <TinyFENDisplay fen={fen} invert={invertBoard} />
  ));
    ...
    <Stepper ref={stepperRef}>
      ...
      <StepperPanel header="Start Position">
        <div className="flex flex-column h-12rem">
          ...
          />
          <div className="mtop-5 flex pt-4 justify-content-between">
            <Button
              label="Back"
              severity="secondary"
      ...
      <StepperPanel header="Review">
        <section className="form flex gap-3 flex-wrap">
          <label className={"p-3 gap-1 flex flex-column font-bold"}>
            Match Depth
            <InputNumber
              value={repertoireDepth}
              onValueChange={(e: any) => setRepertoireDepth(e.value)}
              placeholder="Repertoire Depth"
            />
          </label>
          <label className={"p-3 gap-1 flex flex-column font-bold"}>
            Name
