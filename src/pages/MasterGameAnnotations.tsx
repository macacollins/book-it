...
const makeKey = (master: string, id: string) =>
  master + "-" + id + "-annotations";
...
function retrieveAnalysis(master: string, id: string) {
  const key = makeKey(master, id);
...
export default function MasterGameAnnotations() {
  ...
  useEffect(() => {
    async function getGame() {
      const morphyGames = await getItemDexie(master + "Games");
  ...
  const [notes, setNotes] = useState(retrieveAnalysis(master, id));

  useEffect(() => {
    saveAnalysis(master, id, notes);
  }, [notes]);
            ...
            {showControls && (
              <>
                ...
                <div className="m-2">
                  ...
                  />
                  {kvHeaders &&
                    `${kvHeaders.White} vs. ${kvHeaders.Black} ${kvHeaders.Event} ${kvHeaders.Date}`}
                </div>
              </>
            )}
            <InputTextarea
