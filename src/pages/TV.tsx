...
export default function TV() {
    ...
    const [ lastGameStartTime, setLastGameStartTime ] = useState(new Date().getTime());
    ...
    useEffect(() => { 
        (async () => {
            ...
            const results = await fetch("https://explorer.lichess.ovh/lichess?" + params, {
                "method": "GET",
    ...
    useEffect(() => {
        ...
        (async function() {
            ...
            for (let index = 0; index < gameIDs.length; index++) {
                ...
                const result = await fetch(finalURL).then(response => response.text());
    ...
    return <div id="viewer-board" className="tv">
        Showing TV for fen = {fen};<br/>
        elapsed time {diff}<br/>
...
function getTVPlan(inputPGN: ParsedPGN): TVPlan {
    ...
    let timeControlBase = 0, timeControlIncrement = 0;

    inputPGN?.headers?.forEach((header: pgnParser.Header) => {
        if (header.name === "TimeControl") {
            [ timeControlBase, timeControlIncrement ] = header.value.split("+").map(Number);
        }
    });

    //debugger;

    let blackClockElapsed = 0, whiteClockElapsed = 0;

    let duration = inputPGN.moves.length * timeControlIncrement + timeControlBase;

    const newMoves: TimedMove[] = inputPGN.moves.map((move: pgnParser.Move, index: number) => {
        let time;
        try {
            // @ts-expect-error yah
        ...
        try {
            // @ts-expect-error yah
            time = (/.*%clk ([0]+:[0-9][0-9]:[0-9][0-9]).*/g).exec(move.comments[0].text)[1]
        } catch (e) {
            debugger;
        }
        ...
        const totalTimeWithIncrements = (timeControlBase) + index * timeControlIncrement; 
