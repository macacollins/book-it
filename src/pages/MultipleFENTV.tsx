...
export default function MultipleFENTV({fens, closeHandler, startInverted = false}: {fens: string[], closeHandler?: () => void, startInverted?: boolean}) {
            ...
            if (currentTVPlan) {
                ...
                if ((newDiff > currentTVPlan.totalDuration) || nextRequested) {
                    ...
                    if (gameQueue.length > 1) {
                        ...
                        setCurrentTVPlan(getTVPlan(gameQueue[0]));
                    } else {
                ...
                const filteredMoves = currentTVPlan?.moves
                    .filter(move => move.numberSeconds < newDiff);
    ...
    }, [currentTVPlan, setGameQueue,setResultPGNs, setCurrentTVPlan, setCurrentTime, setFinallyStarted, nextRequested, setNextRequested]);

    const [ firstLoad, setFirstLoad ] = useState(false);
    // actually load the data
    useEffect(() => { 
        
        async function retrieveGameIDs(remainingFENs: string[], accumulator: string[] = []) {
            if (!loading) {
                return;
            }
            ...
            const [ cachedValue, cacheTimestamp ] = 
                [ cachedValueKey, cachedValueKey ].map(key => localStorage.getItem(key)).map(value => {
                    if (value) {
                ...
                if (new Date().getTime() > cacheTimestamp + (4 * 1000 * 60 * 60)) {
                    ...
                    fetch(finalURL).then(response => response.json()).then(json => {
                        if (json) {
                            ...
                            localStorage.setItem(cacheTimestampKey, JSON.stringify(new Date().getTime()));
                        }
                    }).catch(error => {
            ...
            // wow
            const newGameIDs = [...new Set([...accumulator, ...results.recentGames.map((game: any) => game.id) ])];
    ...
    useEffect(() => {
        ...
        (async function() {
            ...
            for (let index = 0; index < gameIDs.length; index++) {
                ...
                const finalURL = "https://lichess.org/game/export/" + id + "?clocks=true";
    ...
    const controlButtons = <section className="buttons-section flex flex-column gap-3 px-18">
        <Button onClick={() => {setInverted(!inverted)}} label="Flip" />
        <Button onClick={() => {setNextRequested(true)}} label="Next Game" />
        <Button onClick={closeHandler} label="Back" />
        <Button onClick={() => boardRef.current?.resize()} label="Resize" />
        <Button onClick={() => setShowHeaders(!showHeaders)} label="Show Headers" />
    </section>;
    ...
    if (currentTVPlan) {
        ...
        if (filteredMoves.length) {
            let { whiteClockSeconds, blackClockSeconds, player } = filteredMoves.slice(-1)[0];

            const offset = newDiff - whiteClockSeconds - blackClockSeconds;
            console.log("offset", offset);
            
            const whiteOffset = player === 'white' ? offset : 0;
            const blackOffset = player === 'white' ? 0 : offset;
            console.log("white offset", whiteOffset, "black offset", blackOffset, "player", player);

            const format = (seconds:number) => {
                return `${Math.floor(seconds / 60)}:${Math.floor(seconds % 60) < 10 ? "0" : ""}${Math.floor(seconds % 60)}`
            }

            whiteClock = format(currentTVPlan.timeControlBase + 
                (currentTVPlan.timeControlIncrement * Math.floor(filteredMoves.length / 2)) - blackClockSeconds - whiteOffset);

            blackClock = format(currentTVPlan.timeControlBase + 
                (currentTVPlan.timeControlIncrement * filteredMoves.length) - whiteClockSeconds - blackOffset);
        }
    }

    const clock = <>
    <br/>
        <label><strong>White</strong></label> {whiteClock}<br></br>
        <label><strong>Black</strong></label> {blackClock}<br></br>
    </>
    
    return <div className="tv flex row-gap-3 gap-3 p-5">
